...
*This prompt reigned the AI in. It immediately stripped away the custom gradients and implemented Spotify's exact design system: `#121212` backgrounds, the signature `#1ed760` green, rounded floating panels, and a responsive mobile layout.*
...
**The Tweak:** I learned that the iTunes Search API actually supports native cross-origin requests. We tweaked the JavaScript to bypass the proxy entirely and hit `https://itunes.apple.com/search` directly, resulting in a much faster and more reliable fetch.
...
**The Tweak:** Instead of writing complex regex to rewrite URLs in JavaScript, we used a powerful HTML meta tag:
`<meta http-equiv="Content-Security-Policy" content="upgrade-insecure-requests">`
This single line of code forced the browser to automatically upgrade all HTTP requests to HTTPS, instantly fixing the broken images.

### 3. CSS Grid & Micro-Animations
To nail the "Spotify feel," layout isn't enough—interaction matters. We spent time tweaking the CSS to include micro-animations. By utilizing CSS Grid for the home page cards, and adding subtle `transform` and `opacity` transitions on hover, we successfully recreated the iconic green play button that slides up over the album art.
